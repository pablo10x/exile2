...
namespace Exile.Inventory {
    public interface IInventoryController {
        ...
        Action<IInventoryItem, IInventoryItem> onItemSplitted { get; set; }
    }
    ...
    public class InventoryController : MonoBehaviour, IPointerDownHandler, IBeginDragHandler, IDragHandler, IEndDragHandler, IPointerExitHandler, IPointerEnterHandler, IInventoryController {
        ...
        /// <inheritdoc />
        public Action<IInventoryItem, IInventoryItem> onItemSplitted { get; set; }

        public Action<IInventoryItem, int> onQuantityChanged { get; set; }
        ...
        public void OnEndDrag(PointerEventData eventData) {
            ...
            switch (mode) {
                ...
                case InventoryDraggedItem.DropMode.QuantityChanged:
                    onQuantityChanged?.Invoke(_itemToDrag, _itemToDrag.Quantity);
                    break;
                default:
                    Debug.Log($"not handled {mode.ToString()}");
                    break;
            }
        ...
        private bool HandleSwapOnDrop(PointerEventData eventData) {
            ...
            var targetController = raycastResults.Count > 0
                                       ? raycastResults[0]
        ...
        private void HandleSplitStack(IInventoryItem itemToSplit) {
            ...
            if (newStackQuantity > 0) {
                ...
                var newItem = itemToSplit.CreateInstance(-1);
                ...
                if (inventory.GetFirstPointThatFitsItem(newItem, out emptyPoint)) {
                    ...
                    onItemSplitted?.Invoke(itemToSplit, newItem);
                }
        ...
        private Vector2 ScreenToLocalPositionInRenderer(Vector2 screenPosition) {
            RectTransformUtility.ScreenPointToLocalPointInRectangle(inventoryRenderer.rectTransform,
                                                                    screenPosition,
