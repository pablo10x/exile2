...
public class CharacterCam : MonoBehaviour
{
    ...
    {
        UpdateWithInput(Time.deltaTime, zoomInput, uiDrag != null ? uiDrag.Delta * CamSensitivity : Vector3.zero);
    }
    ...
    private void UpdateVehicleCamera(float deltaTime, float zoomInput, Vector3 rotationInput)
    {
        ...
        else if (_character.playerCar != null && Time.time - _lastRotationInputTime > 1.6f && _character.playerCar.controllerV4.speed > 35f)
        {
            ...
            PlanarDirection = Vector3.Lerp(PlanarDirection, _character.playerCar != null && _character.playerCar.controllerV4.direction == -1 ? -FollowTransform.forward : FollowTransform.forward, deltaTime * VehicleRotationSharpness);
        }
    ...
    private void ApplyCameraTransform(float deltaTime)
    {
        ...
        var targetRotation = _character.CurrentCharacterState is Character.CharacterState.InVehicleDriver or Character.CharacterState.InVehiclePassenger ? Quaternion.Slerp(Transform.rotation, planarRot * verticalRot, 1f - Mathf.Exp(-VehicleRotationSharpness * deltaTime)) : Quaternion.Slerp(Transform.rotation, planarRot * verticalRot, 1f - Mathf.Exp(-RotationSharpness * deltaTime));
