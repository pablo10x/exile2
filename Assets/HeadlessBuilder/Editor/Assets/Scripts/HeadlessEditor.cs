        ...
        if (currentView == VIEW_SETTINGS)
        {
            ...
            if (headlessSettings.targetSection)
            {
                ...
                if (headlessSettings.valueOverrideTarget)
                {
                    ...
                    GUILayout.Space(6);
                    EditorGUILayout.BeginHorizontal();
                    newSettings.valueDedicated = EditorGUILayout.ToggleLeft("", headlessSettings.valueDedicated, GUILayout.MaxWidth(13f));
                    EditorGUILayout.LabelField("Dedicated Server", codeLabelStyle);
                    EditorGUILayout.EndHorizontal();
                    GUILayout.Space(3);
                    GUILayout.Label("//Check this to build as a dedicated server (Unity 2021.2 and later).", commentStyle);
            ...
            if (headlessSettings.applicationSection)
            {
...
#else
				EditorGUI.BeginDisabledGroup(true);
				newSettings.valueConsole = EditorGUILayout.ToggleLeft ("", false, GUILayout.MaxWidth(13f));
				EditorGUI.EndDisabledGroup();
#endif
                ...
                GUILayout.Space(3);
                GUILayout.Label("//Override the default Unity logging (Unity 2019.1 and earlier).", commentStyle);
                EditorGUILayout.Space();
                EditorGUILayout.EndVertical();
                EditorGUILayout.EndVertical();
...
#else
                EditorGUI.BeginDisabledGroup(true);
                newSettings.valueShader = EditorGUILayout.ToggleLeft("", false, GUILayout.MaxWidth(13f));
                EditorGUI.EndDisabledGroup();
#endif
...
#pragma warning restore 0168
    ...
    public static void SaveSettings(HeadlessSettings newSettings, string profilePath)
    {
        ...
        //headlessSettings = newSettings;
        newSettings.name = "HeadlessSettings";
        ...
        newRuntime.name = "HeadlessRuntime";
