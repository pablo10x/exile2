...
namespace Salvage.ClothingCuller.Editor
{
    public class ViewableMesh
    {
        ...
        private Renderer createRendererCopy(Renderer originalRenderer, Transform targetParent, Transform rigRootTransform)
        {
            ...
            if (rigRootTransform != null && originalRenderer is SkinnedMeshRenderer originalSkinnedMeshRenderer)
            {
                ...
                foreach (Transform child in rendererCopy.transform)
                {
                ...
                {
                    if (component != rendererCopy && component != rendererCopy.transform)
                    {
            ...
            else
            {
                ...
                newGO.transform.position = originalRenderer.transform.position;
        ...
        public void CreateOcclusionDataIfNotExists()
        {
            meshOcclusionData.CreateTriangleOcclusionDataIfNotExists(occludingViewableLOD.LodOcclusionData.Id);
        }

        public bool HasOcclusionData()
        {
            return meshOcclusionData.HasOcclusionData(occludingViewableLOD.LodOcclusionData.Id);
        }
        ...
        public void SetOcclusionDataForTriangle(int triangleIndex, bool isTriangleOccluded)
        {
            if (!meshOcclusionData.TryGetTriangleOcclusionData(occludingViewableLOD.LodOcclusionData.Id, out bool[] triangleOcclusionData))
            {
                return;
        ...
        public void SetOcclusionDataForCircle(Vector3 point, bool occlude, Vector3 cameraPosition)
        {
            if (!meshOcclusionData.TryGetTriangleOcclusionData(occludingViewableLOD.LodOcclusionData.Id, out bool[] triangleOcclusionData))
            {
                return;
        ...
        public void SetOcclusionDataForSphere(Vector3 point, bool occlude)
        {
            if (!meshOcclusionData.TryGetTriangleOcclusionData(occludingViewableLOD.LodOcclusionData.Id, out bool[] triangleOcclusionData))
            {
                return;
        ...
        public void InvertOcclusionData()
        {
            if (!meshOcclusionData.TryGetTriangleOcclusionData(occludingViewableLOD.LodOcclusionData.Id, out bool[] triangleOcclusionData))
            {
                return;
            ...
            for (int i = 0; i < triangleOcclusionData.Length; i++)
            {
                triangleOcclusionData[i] = !triangleOcclusionData[i];
            }
        ...
        public void DrawCircle(Vector3 point, Vector3 normal)
        {
            Handles.zTest = CompareFunction.Disabled;
            Handles.color = Color.yellow;
            Handles.DrawWireDisc(point, normal, OcclusionDataViewerConfiguration.PersistentValues.PaintRadius);
        }
        
        public void DrawSphere(Vector3 point)
        {
            Handles.zTest = CompareFunction.Disabled;
            Handles.color = Color.yellow;
            Handles.RadiusHandle(Quaternion.identity, point, OcclusionDataViewerConfiguration.PersistentValues.PaintRadius);
        }
