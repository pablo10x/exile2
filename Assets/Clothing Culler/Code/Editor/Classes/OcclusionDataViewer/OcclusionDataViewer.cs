...
namespace Salvage.ClothingCuller.Editor
{
    ...
    public static class OcclusionDataViewer
    {
        ...
        private static void onSceneGUI(SceneView sceneView)
        {
            if (occludeeLODs != null && occluderLODs != null)
            {
                ...
                if (Tools.viewTool != ViewTool.Orbit)
                {
                    ...
                    {
                        switch (OcclusionDataViewerConfiguration.PersistentValues.SelectedBrushType)
                        {
                            case OcclusionDataViewerConfiguration.BrushType.Triangle:
                                handleFaceSelection(sceneView);
                                break;
                            case OcclusionDataViewerConfiguration.BrushType.Circle:
                                case OcclusionDataViewerConfiguration.BrushType.Sphere:
        ...
        private static void drawControlsWindow(SceneView sceneView)
        {
            ...
            if (OcclusionDataViewerConfiguration.PersistentValues.SelectedBrushType == OcclusionDataViewerConfiguration.BrushType.Circle || 
                OcclusionDataViewerConfiguration.PersistentValues.SelectedBrushType == OcclusionDataViewerConfiguration.BrushType.Sphere)
            {
                targetHeight += 39;
            }
        ...
        private static void drawPaintButton()
        {
            ...
            GUILayout.Label("Brush type", GUILayout.Width(160));
            var selectedIndex = (int)OcclusionDataViewerConfiguration.PersistentValues.SelectedBrushType;
            var displayedOptions = System.Enum.GetNames(typeof(OcclusionDataViewerConfiguration.BrushType));
            OcclusionDataViewerConfiguration.PersistentValues.SelectedBrushType = (OcclusionDataViewerConfiguration.BrushType)EditorGUILayout.Popup(selectedIndex, displayedOptions, popupStyle, GUILayout.Width(148));
        ...
        private static void drawRadiusSlider()
        {
            ...
            GUILayout.Label("    Hold <b>CTRL</b> to deselect triangles", labelStyle, GUILayout.Width(160 * 2));
        ...
        private static void onInvertButtonClicked()
        {
            selectedOccludeeLOD.InvertOcclusionData();
        }
            ...
            switch (Event.current.type)
            {
                ...
                case EventType.Repaint:
                    if (clickedViewableMesh != null)
                    {
                        ...
                        clickedViewableMesh.DrawTriangle(clickedViewableMesh.SelectedTriangleIndex.Value);
                    ...
                    selectedOccludeeLOD.GetViewableMesh(hoveredMeshCollider).DrawTriangle(hoverHitInfo.triangleIndex);
                    break;
        ...
        private static void handleFaceSelection(SceneView sceneView)
        {
            switch (Event.current.type)
            {
                ...
                case EventType.Repaint:
                    ...
                    hoveredViewableMesh.DrawTriangle(hoverHitInfo.triangleIndex);
                    break;
                case EventType.MouseDown:
                    if (Event.current.button != 0 || controlsWindowRect.Contains(Event.current.mousePosition))
                    {
                    ...
                    clickedViewableMesh.SetOcclusionDataForTriangle(clickHitInfo.triangleIndex, !wasClickedTriangleOccluded);
                    break;
                case EventType.MouseDrag:
                    ...
                    draggedViewableMesh.SetOcclusionDataForTriangle(dragHitInfo.triangleIndex, !wasClickedTriangleOccluded);
                    break;
        ...
        private static void handleRadiusPainting(SceneView sceneView)
        {
            switch (Event.current.type)
            {
                ...
                case EventType.Repaint:
                    ...
                    {
                        case OcclusionDataViewerConfiguration.BrushType.Circle:
                            hoveredViewableMesh.DrawCircle(hoverHitInfo.point, hoverHitInfo.normal);
                            break;
                        case OcclusionDataViewerConfiguration.BrushType.Sphere:
                            hoveredViewableMesh.DrawSphere(hoverHitInfo.point);
                            break;
                    }
                    break;
                case EventType.MouseDown:
                case EventType.MouseDrag:
                    ...
                    {
                        case OcclusionDataViewerConfiguration.BrushType.Circle:
                            clickedViewableMesh.SetOcclusionDataForCircle(clickHitInfo.point, !Event.current.control, sceneView.camera.transform.position);
                            break;
                        case OcclusionDataViewerConfiguration.BrushType.Sphere:
                            clickedViewableMesh.SetOcclusionDataForSphere(clickHitInfo.point, !Event.current.control);
                            break;
                    }
                    break;
            }
        ...
        private static bool tryCreateRig(Renderer[][] renderersByLOD, out Animator animator, out Transform rigRootTransform)
        {
            ...
            if (!OcclusionDataViewerConfiguration.PersistentValues.ViewMeshesAsSkinned)
            {
                return false;
            }
        ...
        private static void onSelectedLODIndexChanged(int selectedLodIndex)
        {
            ...
            selectedOccludeeLOD.CreateOcclusionDataIfNotExists();
