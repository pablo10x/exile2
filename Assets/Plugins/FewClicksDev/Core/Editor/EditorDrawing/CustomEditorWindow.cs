namespace FewClicksDev.Core
{
    ...
    public abstract class CustomEditorWindow : EditorWindow, IHasCustomMenu
    {
        ...
        protected virtual void drawWindowFooter()
        {
            ...
            using (new HorizontalScope())
            {
                float _width = windowWidthScaled(0.9f);

                FlexibleSpace();
                EditorGUILayout.LabelField($"{windowName} :: v{version}", EditorStyles.boldLabel.WithTextAlignment(TextAnchor.MiddleCenter), FixedWidth(_width));
                FlexibleSpace();
            }
